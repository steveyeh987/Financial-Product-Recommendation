...
for r in [4, 5, 6]:
    ...
    def neural_network_model(sequence_length, num_classes, embedding_size, filter_sizes, num_filters, keep_prob): 
        ...
        concat_expanded = tf.expand_dims(concat, 2)
        dwell_embedding_expanded = tf.expand_dims(dwell_embedding, 2)
        concatted = tf.concat([concat_expanded, dwell_embedding_expanded], 2)
        dense = tf.reshape(concatted, [-1, 2*sequence_length, embedding_size])
        time_dense = tf.layers.dense(dense, units=embedding_size, activation=tf.nn.relu)
